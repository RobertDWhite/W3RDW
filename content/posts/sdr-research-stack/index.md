...
The entire data flow — from antenna to searchable, tagged, transcribed recording — runs automatically with no operator input.
...
- Fixed monitor: **144.390 MHz** (APRS primary — permanent, never released)
- Dynamic FM slots: 12
- Dynamic CW slots: 4

### RTL-SDR #2 — 70cm Band

A standard RTL-SDR dongle (serial `00000002`) on **rke2-node-13** handles 70cm. It runs at **2.4 MSPS** centered at **432 MHz**, covering the core 70cm amateur allocation. The DVB kernel driver is blacklisted on this node so the SDR driver loads cleanly.
...
The [RX888 MkII](https://www.rx-sdr.com/) (Cypress WestBridge) on **rke2-node-12** handles HF from 3–28 MHz. It connects via SoapySDDC (`driver=SDDC`) with a 2 MSPS capture rate. This receiver does not support multiple simultaneous clients, so the HF decoders time-share it using a cooperative scheduler (described in the HF section below).
...
Minimum output threshold is 2 characters (to filter pure noise triggers). CW recordings are captured on any frequency in the ham band where the FFT detects a narrowband signal — the decoder figures out whether there is actually Morse there.

### Pager — POCSAG and FLEX

A separate RTL-SDR (serial `00000001`) on rke2-node-13 monitors the pager band. The `unified_sdr.py` script routes recordings from 151.820–154.200 MHz and 157.450–158.700 MHz to `/data/audio/pager/` instead of the voice directory. The pager decoder runs `multimon-ng` in POCSAG512, POCSAG1200, POCSAG2400, and FLEX modes against these files, writing decoded messages to `/data/text/pager/`. Retention is 30 days.
...
Before transcription, it excludes APRS frequencies (144.37–144.41 MHz and 145.805–145.845 MHz) — those go to the APRS decoder, not Whisper.
...
After transcription, a regex filter catches common Whisper hallucinations on noise — phrases like "Thank you for watching" and similar — and marks those recordings as no-speech rather than storing a false transcript.
...
The SDR research platform includes a live ADS-B feed. The ADS-B stack (ultrafeeder + Flightradar24 feeder) runs separately on rke2-node-15 with a dedicated 1090 MHz antenna, publishing data at [adsb.w3rdw.radio](https://adsb.w3rdw.radio).
...
This means recordings are searchable by repeater name, trustee, and location — not just raw frequency. The repeater data also feeds the Ollama prompt context, so the tagger knows whether a transmission was on a linked repeater, a simplex frequency, or a public safety channel.
...
**VMware USB passthrough does not work for the Airspy Mini.** The Airspy returns error -71 (EPROTO) when accessed through a VMware hypervisor's USB passthrough. Direct bare-metal USB is required. The RTL-SDRs work fine via VMware passthrough.
...
**Whisper large-v3 hallucinates on noise.** On weak or empty signals, the model confidently produces fluent sentences that have nothing to do with the audio. "Thank you for watching" is the most common. Aggressive VAD filtering and a post-processing regex filter are necessary to keep false transcripts out of the database.

**The RX888 MkII is single-client only.** Unlike the Airspy (which supports SoapyRemote sharing), the RX888 via SoapySDDC allows only one active connection. Any multi-decoder HF setup requires time-sharing at the application layer.
...
> As always, if you have any questions or want to contribute to the above information, feel free to start a [Discussion on GitHub](https://github.com/RobertDWhite/W3RDW/discussions), [submit a GitHub PR](https://github.com/RobertDWhite/W3RDW/pulls) to recommend changes or fixes, or reach out to me directly at [robert@w3rdw.radio](mailto:robert@w3rdw.radio). Feel free to [join my Matrix channel for W3RDW](https://matrix.to/#/%23w3rdw:white.fm) and chat with me there.
>
> Thanks for reading!
>
