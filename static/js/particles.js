/* ============================================================================
   SIGNAL / HF — contact network
   ----------------------------------------------------------------------------
   Sibling of the particle layer on whitematter.tech. There the nodes are
   neurons and the travelling dots are impulses; here they are stations and the
   dots are contacts propagating between them. The home page already draws that
   analogy in words — this draws it.

   Roughly one contact in nine is worked in the horizon amber rather than the
   propagation blue: the DX, the one that went further than it should have.

   This file replaces the old particles.js config, which also carried a stats.js
   FPS counter copied from the original CodePen. stats.js was never loaded on
   this site, so every page threw "Uncaught ReferenceError: Stats is not
   defined". Gone.
   ========================================================================== */
...
(function () {
    ...
    var SIGNAL = "#4db8ff";   /* propagation blue */
    var SIGNAL_2 = "#8b7bff"; /* waterfall violet */
    var CRIT = "#ffa63d";     /* horizon amber — the DX contact */
    ...
    function start(attempt) {
        ...
        function spawn() {
            ...
            var critical = Math.random() < 0.11; /* the DX */
            packets.push({
                from: from,
